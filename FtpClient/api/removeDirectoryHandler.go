...
func RemoveDirectoryHandler(w http.ResponseWriter, r *http.Request) {
	enableCors(&w)
	...
	stat, err := ftpSession.RMD(request.Path)
	if err != nil {
		js, _ := json.Marshal(ResponseOperation{err.Error(), false})
		responseWrite(&w, js)
		return
	...
	js, _ := json.Marshal(ResponseOperation{"Directory Deleted", true})
	responseWrite(&w, js)
}